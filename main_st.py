...
def train(args, model, device, train_loader, optimizer, iteration, epoch, prompt):
    model.train()
    for batch_idx, (data, label) in enumerate(train_loader):
        ...
        if batch_idx % args.log_interval == 0:
            ...
            print(y_prob[0].data.cpu().numpy().max())
...
def train_entropy(args, model, device, train_loader, optimizer, iteration, epoch, prompt):
    model.train()
...
def main():
    ...
    for i in range(1, args.num_iters+1):
        ...
        for epoch in range(1, args.num_epochs_target+1):
            train_target(args, model, device, tar_loader_train, optimizer, i, epoch, 'Target')
        for param_group in optimizer.param_groups:
